  ...
  class Page
    ...
    class PrefetchEngine
      ...
      private def is_in_viewport?(element : DOM::Element) : Bool
        # 実際の実装ではDOM要素の位置とサイズに基づいて判定
        # ここでは単純化のためにtrue/falseをランダムに返す
        Random.rand > 0.5
      end
    ...
    def save_state : PageState
      ...
      # 新しい状態オブジェクトを作成
      @state = PageState.new(@scroll_position, form_data, selected_elements, js_state)
    end
    ...
    def restore_state(state : PageState) : Bool
      ...
      state.selected_elements.each do |id|
        if elem = @document.not_nil!.get_element_by_id(id)
          elem_type = elem.get_attribute("type")?.to_s
